...
import type { LanyardData, LanyardResponse } from "./restTypes";
import {
  LanyardOpcode,
  type InitState,
  type LanyardEvent,
...
export function useLanyard(
  config: LanyardConfigRest | LanyardConfigOne
...
export function useLanyard(config: LanyardConfig) {
  ...
  if (config.method === "ws") {
    ...
    return readable<Record<string, LanyardData>>(undefined, (set) =>
...
function lanyardWS<T extends InitState>(
  config: LanyardConfigWS,
  set: Subscriber<T>
) {
  if (typeof window === "undefined") {
    return;
  ...
  const recv = (
    callback: (this: WebSocket, event: MessageEvent<string>) => unknown
  ) => {
    ws.addEventListener("message", callback);
  };
  const once = <T>() =>
    new Promise<T>((res) => {
      const fn: (this: WebSocket, event: MessageEvent<string>) => unknown = (
        event
      ) => {
